...
- [ ] `grep -n "createBrowserAiRunHost" src` has a caller outside `browser.ts`
- [ ] Hosted boot does not `setDetectedProviders([])` on capability failure
- [ ] `bun test src/plugins/builtin/ai src/plugins/builtin/sec/summary-runner.test.ts` passes
- [ ] `plans/README.md` row 045 → DONE
