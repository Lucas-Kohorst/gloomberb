...
`CAT [query]` opens the Data Catalog — a searchable, filterable inventory of every chartable series. Securities are listed as `<ticker> - field` for book names without fetching live series. Prediction shows Kalshi/Polymarket; Adjacent indices and VoteHub polls live under Other. Enter or `[g]`raph charts the selected series.
