...
- **Data Catalog** (`CAT [query]`) — searchable inventory of every chartable series, including options and Artificial Analysis. Securities list `<ticker> - field` for book names without fetching series. Prediction shows Kalshi/Polymarket without a search; Adjacent indices live in Other with VoteHub polls. Enter or `[g]`raph charts it; `/` search; `[o]`pen when a series has an external URL.
