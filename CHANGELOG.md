...
- Adjacent Cloud composes Polls, Federal Register, OFAC, and USAspending so `POLL` / `FR` / `OFAC` / `USA` show up in Assist. Their fetch traffic folds onto the Adjacent Cloud Connections row — no extra source ids. `WX` opens the Adjacent pane (climate/weather markets) rather than restoring a broken weather plugin. Congress Trades (`CG`) stays the only extra first-party plugin. Extracted copies with those module ids are reserved.
...
## Unreleased — Stop tracking `plans/`

Implementation notes under `plans/` were committed to the repo. They are local scratch, so they are untracked and gitignored.
