...
describe("study resolution", () => {
  ...
  test("bounds perfectly correlated values despite floating-point rounding", () => {
    const left = resolved("left");
    left.points = left.points.slice(0, 4).map((point, index) => ({ ...point, value: [0.1, 0.2, 0.3, 0.4][index]! }));
    for (const direction of [1, -1]) {
      const right = { ...resolved("right"), points: left.points.map((point) => ({ ...point, value: point.value! * 19 * direction })) };
      const calculation = study("corr", "correlation", ["left", "right"], { period: 4, returns: 0 });
      expect(resolveStudies([left, right], [calculation]).series[0]!.points[0]!.value).toBe(direction);
    }
  });
