...
describe("OnboardingWizard", () => {
  ...
  test("share hand-off opens on sign-up and has no skip", async () => {
    tempDataDir = await mkdtemp(join(tmpdir(), "gloomberb-share-handoff-"));
    const pluginRegistry = createPluginRegistry();
    ...
    // The wizard starts at the Cloud/account section when requireAccount is set.
    let frame = await waitForFrame("Sync across apps");
    expect(frame).not.toContain("Not now");
    expect(frame).not.toContain("this step is optional");

    await emitKeypress({ name: "escape", sequence: "\u001b" });
    frame = await waitForFrame("Sign up free");
