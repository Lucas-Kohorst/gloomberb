...
export function OnboardingWizard({ pluginRegistry, importBrokerPositions, requireAccount = false, onComplete }: OnboardingWizardProps) {
  ...
  const stage = requireAccount && progress.stage === "welcome" ? "account" : progress.stage;
  ...
  }, [account.beginAccountMode, account.beginQrSignIn, choiceIds, saveProgressInBackground]);
  ...
  useShortcut((event) => {
    ...
    if (stage === "account") {
      if (enter) {
        ...
        consume();
        account.setAccountChoiceIdx((index) => Math.min(choiceIds.length - 1, index + 1));
      }
      return;
    }
    if (stage === "upgrade") {
