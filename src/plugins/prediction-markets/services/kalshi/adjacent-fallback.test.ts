...
import { loadKalshiCatalog, loadKalshiHistory, resolveKalshiChartSummary } from "./adapter";
import {
...
describe("Kalshi catalog 429 fallback", () => {
  test("loads Adjacent Kalshi markets when venue event lists are rate-limited", async () => {
    ...
    const markets = await loadKalshiCatalog("", "all");
  ...
  test("resolves a chart ticker through Adjacent when Kalshi event GET is rate-limited", async () => {
    setHttpFetchTransport(async (url) => {
    ...
    const summary = await resolveKalshiChartSummary("KXPRESNOMD-28", "KXPRESNOMD-28-REMA");
