...
export interface FuturesContract {
  ...
  symbol: string;
  /** Exchange code traders use, e.g. ES. */
  code: string;
  name: string;
...
/**
 * Front-month continuous contracts, all confirmed to resolve through the
 * existing Yahoo provider. `DX=F` is deliberately absent — Yahoo 404s it, and
 * the dollar index is already on the world indices board as `DX-Y.NYB`.
 */
