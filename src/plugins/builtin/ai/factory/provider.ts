...
export const FACTORY_AGENT_SYSTEM_PROMPT = [
  ...
  "CFTC DCM chart: {\"type\":\"call\",\"operation\":\"pane.createFromTemplate\",\"input\":{\"templateId\":\"cftc-filings-pane\",\"options\":{\"arg\":\"chart\"}}}",
  ...
  "Politics: {\"type\":\"call\",\"operation\":\"layout.new\",\"input\":{\"name\":\"Democrats\",\"panes\":[\"polls\",\"congress-trades\",\"adjacent\",\"prediction-markets\",\"news-firehose\"]}}",
  ...
  "Known pane and template ids by theme: politics — polls, congress-trades, adjacent, prediction-markets, news-firehose, twitter-feed-pane. markets — portfolio-list, ticker-research, market-movers, fear-greed, sectors, volatility, short-interest, market-halts, earnings-calendar, insider. macro — econ-calendar, yield-curve, world-indices, treasury-auctions, federal-register, credit-conditions, fear-greed, cds, fx-matrix, futures. news — news-firehose, news-top-pane, news-breaking-pane, news-feed-pane, substack-pane, macro-tv-pane, twitter-feed-pane. research — sec, ticker-news, earnings-calendar, ipo-calendar, analyst-research, owid, chart-composer. For a theme not listed here, read app://pane-templates once to discover the right ids, then layout.new with panes[].",
  ...
  "Every plugin setup must call registerAgentPromptFragment (and registerAgentTool when it has unique data) so the agent can open it.",
