...
export function CustomViewPane({ focused, width, height }: PaneProps) {
  ...
  usePaneFooter(CUSTOM_VIEW_PANE_ID, () => ({
    ...
    hints: spec
      ...
      : [],
  }), [data?.errors, data?.sourceLabel, publishToTeam, rows.length, spec]);
  ...
  if (loading && !data) {
    return <PaneStatusBody loading subject="view" />;
  }
  if (error && !data) {
    return (
      <Box width={width} height={height} padding={1} flexDirection="column" gap={1}>
        <EmptyState title="This view could not load." message={error} hint="Press r to retry." onRetry={() => { void reload(); }} />
      </Box>
    );
  }
  if (rows.length === 0) {
