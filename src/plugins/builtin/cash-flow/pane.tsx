...
import { usePaneFooterHintBindings, paneRefreshHint } from "../shared/pane-footer";
...
export function CashFlowPane({
  ...
  const [requestedPeriod, setRequestedPeriod] = useState<CashFlowPeriod>("annual");
  ...
  const footerHints = useMemo(
    () => (symbol ? [paneRefreshHint(reload)] : []),
    [reload, symbol],
  );

  useEffect(() => {
    if (requestedPeriod !== period) setRequestedPeriod(period);
  ...
  return (
    <DataTableView<CashFlowTableRow, CashFlowColumn>
      ...
      rootBefore={(
        <Box flexDirection="row" height={1} paddingX={1} justifyContent="flex-end">
          <SegmentedControl
            focused={focused}
            options={[
              { label: "Annual", value: "annual", disabled: annualStatements.length === 0 },
              { label: "Quarterly", value: "quarterly", disabled: quarterlyStatements.length === 0 },
            ]}
            value={period}
            onChange={(value) => setRequestedPeriod(value as CashFlowPeriod)}
          />
        </Box>
      )}
    />
