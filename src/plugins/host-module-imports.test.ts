...
describe("plugin host module imports", () => {
  test("has a host module for every shared specifier", async () => {
    ...
    expect(registry["gloomberb/plugins"]).toBeTruthy();
  });

  test("rejects specifiers that are not on the host contract", async () => {
    await expect(importPluginHostModule("gloomberb/broker")).rejects.toThrow("Unknown plugin host module");
  });
